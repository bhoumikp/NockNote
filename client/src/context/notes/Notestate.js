...
const Notestate = (props) => {
  ...
  const getAllNotes = async()=>{
      //api call
      const responce = await fetch(`/api/notes/fetchallnotes`, {
        method: 'GET',
        headers: {
  ...
  const Addnote = async(title, description, tag) => {
    //api call
    const responce = await fetch(`/api/notes/addnote`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'auth-token': localStorage.getItem('token')
  ...
  const DeleteNote = async(id) => {
    //api call
    const responce = await fetch(`/api/notes/deletenote/${id}`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
        'auth-token': localStorage.getItem('token')
  ...
  const EditNote = async (id, title, description, tag) => {
    ...
    const responce = await fetch(`/api/notes/updatenote/${id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        'auth-token': localStorage.getItem('token')
